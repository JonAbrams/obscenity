    ...
    class << self
      ...
      def profane?(text)
        ...
        blacklist.each do |foul|
          return(true) if text =~ foul_regex(foul) && !whitelist.include?(foul)
        end
        false
      end
      ...
      def sanitize(text)
        ...
        blacklist.each do |foul|
          text.gsub!(foul_regex(foul), replace(foul)) unless whitelist.include?(foul)
        end
      ...
      def offensive(text)
        ...
        blacklist.each do |foul|
          words << foul if text =~ foul_regex(foul) && !whitelist.include?(foul)
        end
        words.uniq
      end
      ...
      def foul_regex(foul)
        if Obscenity.config.no_boundaries
          /#{foul}/i
        else
          /\b#{foul}\b/i
        end
